...
from openerp import fields as openerpfields
import os
...
class account_invoice(osv.osv):
    ...
    _columns = {
                'rep_id': fields.related('partner_id','parent_id', string='Rep id', type='many2one', relation='res.partner', store=True),
                ...
                'afip_concept': fields.selection([
                                ...
                                ], 'AFIP Concepts', select=True, required=False, readonly=True, states={'draft':[('readonly',False)]}),
                'afip_service_start': fields.date('AFIP Service start', readonly=True, states={'draft':[('readonly',False)]}),
                'afip_service_end': fields.date('AFIP Service end', readonly=True, states={'draft':[('readonly',False)]}),
    ...
    def action_number(self, cr, uid, ids, context=None):
        ...
        for inv in self.browse (cr, uid, ids):
            ...
            if inv.type == "out_invoice":
                ...
                sales_point = inv.journal_document_class_id.point_of_sale_id
                sales_point_id = inv.journal_document_class_id.point_of_sale_id.id
                ...
                if vat_condition == '01' or vat_condition == '11':
                    ...
                    if inv.partner_id.vat_condition == '01' or inv.partner_id.vat_condition == '11':
                        # Responsable inscripto o agente de percepción
                        afip_invoice_type = inv.document_type in ('debit_note') and '002' or '001' # ND A o Fact A
                        ...
                        afip_invoice_type = inv.document_type in ('debit_note') and '020' or '019' # ND E o Fact E
                    else:
                        # Todos los demás: Factura B
                        max_no_id = afip_obj.get_max_b_no_id(cr, uid)
                        # solo controlo el dni para lived
                        if inv.company_id.name != "ALFA":
                        ...
                        afip_invoice_type = inv.document_type in ('debit_note') and '007' or '006' # ND B o Fact B
                    ...
                    if inv.partner_id.vat_condition == '09' or inv.partner_id.vat_condition == '10':
                        ...
                        afip_invoice_type = inv.document_type in ('debit_note') and '012' or '011' # ND C o Fact C
                else:
            ...
            if invtype in ('out_invoice', 'out_refund'):
                afip_sales_point = inv.company_id.point_of_sale_ids.number
                if afip_invoice_type:
                    # afip_number = inv.afip_sales_point_id.next_number(afip_invoice_type)
                    ...
                    afip_number = inv.journal_document_class_id.sequence_id.number_next
                    # printer = inv.shop_id.afip_sales_point_id.get_printer(afip_invoice_type)
                    # dup = inv.shop_id.afip_sales_point_id.is_dup(afip_invoice_type)
                    report = inv.get_report_name()
            ...
            for inv_id, name in self.name_get(cr, uid, [id]):
                ...
                self.log(cr, uid, inv_id, message, context=ctx)
            # if inv.journal_document_class_id.point_of_sale_id.type == "electronic":
            sequence = seq_obj.browse(cr, uid, inv.journal_document_class_id.sequence_id.id)
            sequence.write({'number_next': inv.journal_document_class_id.sequence_id.number_next + 1})
        return True
    ...
    def finalize_invoice_move_lines(self, move_lines):
        ...
        if invoice_browse.type == 'out_invoice':
            ...
            #todo liricus: falta definir esto de la direccion
        ...
        for inv in self.browse (cr, SUPERUSER, ids, context=context):
            for il in inv.invoice_line:
                ...
                for tax in il.invoice_line_tax_id:
                    new_tax = self._convert_tax(cr, SUPERUSER, tax, inv.company_id.id, context=context)
                    if new_tax:
                        new_taxes.add (new_tax.id)
                #todo liricus ok:  se saco el context
                #il.write({'invoice_line_tax_id': [(6, 0, list(new_taxes))]}, context=context)
                il.write ({'invoice_line_tax_id': [(6,0,list(new_taxes))]})
                
                if il.account_id and il.account_id.company_id != inv.company_id.id:
                    new_account = self._convert_account(cr, uid, il.account_id, inv.company_id.id, context=context)
                    # todo liricus ok: se saco el context
                    #il.write({'account_id': new_account and new_account.id}, context=context)
                    il.write({'account_id': new_account and new_account.id})

    def fix_account (self, cr, uid, ids, context=None):
        # todo liricus: Esto verlo despues que ande mas o menos bien la facturacion
        for inv in self.browse(cr, SUPERUSER, ids, context=context):
            invoice_vat_amount = 0
            invoice_vat_base_amount = 0

            if inv.account_id and inv.account_id.company_id != inv.company_id:
                new_account = self._convert_account (cr, SUPERUSER, inv.account_id, inv.company_id.id, context=context)
                inv.write({'account_id': new_account and new_account.id})

            # for line in inv.invoice_line:
            #     invoice_vat_amount += line.vat_amount
            #     invoice_vat_base_amount += line.price_subtotal
            # inv.write({'vat_amount': invoice_vat_amount, 'vat_base_amount': invoice_vat_base_amount})
...
class account_invoice_tax (osv.osv):
    ...
    def compute(self, cr, uid, invoice_id, context=None):
        ...
        # todo liricus comento esto por lo de (1) verificar que vaya bien
        ...
        # todo liricus (1)
        invoice = invoice_id
        tax_grouped = super(account_invoice_tax, self).compute(cr, uid, invoice.id, context=context)
        # todo liricus fin (1)

        company = invoice.journal_id.company_id
        total_wo_taxes = invoice.amount_untaxed
        partner = invoice.partner_id
        #todo liricus comento address = invoice.address_invoice_id
        ...
        for addtax in tax_obj.browse(cr, uid, [x.id for x in tax_list]):
            #todo liricus saco address.id no se si influye luego,
