...
class report_document_receipt(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(report_document_receipt, self).__init__(cr, uid, name, context)
    ...
    def get_doc_desc(self, doc):
        if doc.type == 'cheque':
            s = "Banco: %s, Cheque Nro: %s, Titular: %s, CUIT: %s, Venc: %s" % (doc.issuer.bank.name, doc.name, doc.issuer.owner_name, doc.issuer.cuit, doc.maturity_date)
        elif doc.type == 'pagare':
            s = "Pagaré Titular: %s, Nro: %s, CUIT: %s, Venc: %s" % (doc.issuer.owner_name, doc.name, doc.issuer.cuit, doc.maturity_date)
        else:
            s = "Doc.a Pagar Titular: %s, Nro: %s, CUIT: %s, Venc: %s" % (doc.issuer.owner_name, doc.name, doc.issuer.cuit, doc.maturity_date)

        return s


report_sxw.report_sxw(
    'report.document_receipt_print',
    'numa_ar_base.document_receipt',
    'addons/numa_uniqs_customer_voucher/report/document_receipt.rml',
    parser=report_document_receipt,header="external"
)
