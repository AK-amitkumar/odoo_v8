...
class voucher(osv.osv):
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type=False, context=None, toolbar=False, submenu=False):
        ...
        res = super(voucher, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context,
    ...
    #     vals = self.onchange_journal(cr, uid, ids, journal_id, line_ids, tax_id, partner_id, time.strftime('%Y-%m-%d'),
    ...
    def action_compute_debts(self, cr, uid, ids, context=None):
        ...
        if company_currency != v.currency_id.id and v.type == 'payment':
            total_debit = currency_pool.compute(cr, uid, v.currency_id.id, company_currency, total_debit,
                                                context=context_multi_currency)
        elif company_currency != v.currency_id.id and v.type == 'receipt':
            total_credit = currency_pool.compute(cr, uid, v.currency_id.id, company_currency, total_credit,
                                                 context=context_multi_currency)
        ...
        for line in moves:
            ...
            amount_unreconciled = currency_pool.compute(cr, uid,
                                                        line.currency_id and line.currency_id.id or company_currency,
                                                        v.currency_id.id, abs(line.amount_residual_currency),
                                                        context=context_multi_currency)
            rs = {
                ...
                'amount_original': currency_pool.compute(cr, uid,
                                                         line.currency_id and line.currency_id.id or company_currency,
                                                         v.currency_id.id, line.currency_id and abs(
                        line.amount_currency) or original_amount, context=context_multi_currency),
            ...
            if line.credit:
                amount = min(amount_unreconciled,
                             currency_pool.compute(cr, uid, company_currency, v.currency_id.id, abs(total_debit),
                                                   context=context_multi_currency))
                rs['amount'] = amount
                total_debit -= amount
            else:
                amount = min(amount_unreconciled,
                             currency_pool.compute(cr, uid, company_currency, v.currency_id.id, abs(total_credit),
                                                   context=context_multi_currency))
                rs['amount'] = amount
                total_credit -= amount
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        ...
        if (v.type == 'receipt' and len(crs) == 0) or \
            ...
            if company_currency != v.currency_id.id:
                full_credit = currency_pool.compute(cr, uid, v.currency_id.id, company_currency, full_credit,
                                                    context=context)
            ...
            for line in moves:
                ...
                rs = {
                    ...
                    'amount_original': currency_pool.compute(cr, uid,
                                                             line.currency_id and line.currency_id.id or company_currency,
                                                             v.currency_id.id, line.currency_id and abs(
                            line.amount_currency) or original_amount, context=context),
                    'date_original': line.date,
                    'date_due': line.date_maturity,
                    'amount_unreconciled': amount_unreconciled,
                }
            ...
            # v.write({
            #     'writeoff_amount': self._compute_writeoff_amount(cr, uid, line_dr_ids, line_cr_ids, v.amount),
            #     'line_ids': [(0, 0, x) for x in line_ids],
            #     'pre_line': pre_line,
            # })
